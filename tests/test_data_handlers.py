...
def test_write_csv(tmp_path, sample_df):
    ...
    # Replace None with np.nan in the sample DataFrame
    test_df = sample_df.copy()
    test_df = test_df.replace({None: np.nan})

    # Write and read the DataFrame
    DataHandler.write_csv(test_df, output_path)
    assert output_path.exists()

    # Read and verify
    df_read = pd.read_csv(output_path)

    # Verify specific aspects instead of using assert_frame_equal
    assert df_read.shape == test_df.shape
    assert all(df_read.columns == test_df.columns)

    # Compare non-null values
    for column in test_df.columns:
        mask = test_df[column].notna()
        assert (df_read[column][mask] == test_df[column][mask]).all()
...
def test_convert_dtypes(sample_df):
    ...
    dtype_map = {
        'id': str,
        'value': int  # This will raise an error due to NaN
    }
